...
async function drawTicket(doc: jsPDF, ticket: TicketData) {
    ...
    const titleLines = doc.splitTextToSize(ticket.eventName, width - 10);
    ...
    doc.setFont("helvetica", "normal");
    doc.text(ticket.attendeeName, margin, y + 4);
    y += 10;
    ...
    doc.setFont("helvetica", "normal");
    doc.text(ticket.zone, margin, y + 4);
    ...
    doc.setFont("helvetica", "normal");
    const locLines = doc.splitTextToSize(ticket.location, width - 10);
